...
RUN cd /tmp && git clone https://github.com/zeromq/czmq && cd czmq && ./autogen.sh && ./configure && make -j 4 && make install && ldconfig
...
RUN cd /tmp/booksim2-GenXYZ/ && g++ main2.cpp -o booksim -lczmq -lzmq -lpthread && cp ./booksim /usr/bin/booksim
